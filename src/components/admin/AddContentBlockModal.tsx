...
import cmsAPI, { CreateContentBlockData } from '../../services/cmsAPI';
...
const AddContentBlockModal: React.FC<AddContentBlockModalProps> = ({ onClose, onSuccess }) => {
  const [formData, setFormData] = useState<CreateContentBlockData>({
    name: '',
    type: 'text',
    content: {},
    is_global: false
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await cmsAPI.createContentBlock(formData);
      onSuccess();
    } catch (error) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-semibold text-gray-900">Add Content Block</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="bg-gray-50 p-4 rounded-md border-2 border-dashed border-gray-300">
                ...
                <p className="text-center text-sm text-gray-600">
                  {formData.type.charAt(0).toUpperCase() + formData.type.slice(1)} Block
                </p>
                <p className="text-center text-xs text-gray-500 mt-1">
                  Preview will be available after creation
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-50 p-4 rounded-md">
            <h4 className="text-sm font-medium text-blue-900 mb-2">
              {formData.type.charAt(0).toUpperCase() + formData.type.slice(1)} Block Structure
            </h4>
            <pre className="text-xs text-blue-800 bg-blue-100 p-2 rounded overflow-x-auto">
              {JSON.stringify(getDefaultContent(formData.type), null, 2)}
            </pre>
          </div>
