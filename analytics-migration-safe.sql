...
CREATE INDEX IF NOT EXISTS idx_funnel_analytics_funnel_name ON funnel_analytics(funnel_name);
CREATE INDEX IF NOT EXISTS idx_user_events_user_id ON user_events(user_id);
CREATE INDEX IF NOT EXISTS idx_user_events_event_type ON user_events(event_type);

-- Create view for admin users with analytics data (FIXED to join with auth.users for email)
...
SELECT 
    ...
    p.role,
    CASE 
        WHEN p.role IN ('admin', 'super_admin', 'editor', 'viewer') THEN true
        ELSE false
    END as is_active,
...
FROM profiles p
JOIN auth.users au ON p.id = au.id  -- Join with auth.users to get email
...
DO $$ 
BEGIN
    -- Drop and recreate policies for user_analytics
    DROP POLICY IF EXISTS "Admin users can view all analytics" ON user_analytics;
    ...
    -- Drop and recreate policies for page_analytics
    DROP POLICY IF EXISTS "Admin users can manage page analytics" ON page_analytics;
    DROP POLICY IF EXISTS "Public can insert page analytics" ON page_analytics;
    DROP POLICY IF EXISTS "Public can update page analytics" ON page_analytics;
    ...
    DROP POLICY IF EXISTS "Admin users can view all logs" ON system_logs;
    ...
    -- Drop and recreate policies for funnel_analytics
    DROP POLICY IF EXISTS "Admin users can view all funnel analytics" ON funnel_analytics;
    DROP POLICY IF EXISTS "Users can insert their own funnel analytics" ON funnel_analytics;
    ...
    DROP POLICY IF EXISTS "Admin users can view all events" ON user_events;
...
DROP TRIGGER IF EXISTS update_page_analytics_trigger ON user_analytics;
